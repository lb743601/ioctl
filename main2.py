...
class MultiCameraCapture:
    ...
    def open_camera(self, index, high_resolution=False):
        """打开指定索引的相机"""
    ...
    def capture_all_cameras(self):
        ...
        for idx in self.camera_indices:
            try:
                # 以高分辨率打开当前相机
                self.open_camera(idx, high_resolution=True)
                # 等待相机稳定
                #time.sleep(0.01)
    ...
    def preview_loop(self):
        ...
        try:
            # 初始化预览第一个相机
            self.open_camera(self.camera_indices[self.preview_index], high_resolution=False)
